...
- `llm_judge.py` — the app's first outbound third-party network call: builds a scoring prompt and a per-criterion JSON tool schema (pure functions, testable with zero mocking), calls the Anthropic API with a forced `tool_choice` to get structurally-guaranteed per-criterion scores, and produces an `EvaluationCreate` that `routers/evaluations.py`'s `create_auto_evaluation` (`POST /evaluations/auto`) hands to the existing `create_evaluation_from_payload` unchanged — this module's only job is producing valid input for that path, not duplicating its validation/scoring/persist logic. Like `auth.py`, this is a documented deviation from the pure-module pattern: `AnthropicJudgeClient` does real I/O, exposed as a FastAPI dependency (`get_judge_client`) specifically so tests can override it via `app.dependency_overrides` the same way `auth.py`'s dependencies are overridden — no real Anthropic API calls happen in `uv run pytest`, and no `ANTHROPIC_API_KEY` is needed in CI. Requires `ANTHROPIC_API_KEY` (raises a clear `503` if missing, checked lazily on first use, never at import time); model is `ANTHROPIC_JUDGE_MODEL`, falling back to `claude-haiku-4-5-20251001`.
...
- `auth.py` — a fourth example of the pure-module + thin-router pattern, with one documented deviation: its session functions (`create_session`, `resolve_session_token`, `invalidate_session`) do their own DB I/O rather than staying pure, because that I/O is needed identically by three call sites (JSON login/logout, dashboard form login/logout, and the `get_current_user`/`get_current_dashboard_user` dependencies) — putting it in one place avoids duplicating the same queries three times. Also holds `hash_password`/`verify_password` (bcrypt), `authenticate_user` (deliberately returns `User | None` without distinguishing "no such user" from "wrong password," to avoid username enumeration — callers must show one generic error), and the `DashboardAuthRequired` exception used by the redirect-to-login mechanism. `routers/auth.py` has three `APIRouter`s in one file (justified by URL-prefix constraints): `/auth/login`+`/auth/logout` (JSON), unprefixed `POST /users` (authenticated-only, no roles/admin), and unprefixed `GET/POST /login`+`POST /logout` (dashboard forms) — the dashboard form routes call the same `authenticate_user` core as the JSON login route rather than duplicating credential-check logic. Sessions are server-side rows (`AuthSession` table, opaque `secrets.token_urlsafe` tokens), not JWT — see the README's Authentication section for why. Note: `AuthSession.expires_at` needs its `tzinfo` re-attached after reading back from SQLite (see the comment in `resolve_session_token`) — SQLite drops timezone info on datetime round-trip, which only matters here because this is the first place the codebase compares a stored datetime rather than just displaying it.
