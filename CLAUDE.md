...
- `routers/dashboard.py` — server-rendered HTML routes under `/dashboard` (Jinja2 templates in `templates/`, static assets in `static/`, mounted via `StaticFiles` at `/static` in `main.py`). This one has no pure-calculation counterpart: it's read-only presentation over existing data, and reuses `build_rubric_response` (from `rubrics.py`) and `build_evaluation_responses` (from `evaluations.py`) rather than introducing new business logic. Prompt/response queries have no existing builder either, so those are inlined directly in the route functions.
...
The README's "Future Roadmap" section is the current backlog: comparison charts on the dashboard, CSV import/export, model/provider metadata + cost tracking, cross-rubric analytics, auth, and Postgres support.
